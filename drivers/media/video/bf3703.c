...
#define SENSOR_NAME RK29_CAM_SENSOR_BF3703
#define SENSOR_V4L2_IDENT V4L2_IDENT_BF3703
		...
		case Sensor_PowerDown:
		{
			if (icl->powerdown) {
				...
				if (ret == RK29_CAM_IO_SUCCESS) {
					if (on == 0) {
						mdelay(20);    // 2 modify by sp_yjp,20120831 mdelay(2);
						if (icl->reset)
							icl->reset(icd->pdev);
...
{
    struct i2c_client *client = v4l2_get_subdevdata(sd);//sd->priv;
			       ...
			       struct i2c_client *client)
{
   ...
   /* ret = sensor_write(client, 0x12, 0x80);
    ...
    ret = sensor_read(client, 0xfc, &pid);
    if (ret != 0) {
