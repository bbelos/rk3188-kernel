...
static int pwm_set_rate(struct pwm_platform_data *pdata,int nHz,u32 rate,int gdc_id)
{
	u32 lrc, hrc;
	int id = pdata->pwm_id;
	unsigned long clkrate;

	clkrate = clk_get_rate(g_dcdc[gdc_id]->pwm_clk);
	
	DBG("%s:id=%d,rate=%d,clkrate=%d\n",__func__,id,rate,clkrate); 
    	
    if((pdata->pwm_id == 1)){
   //    printk("%s:id=%d,rate=%d,clkrate=%d\n",__func__,id,rate,clkrate);
       if(rate<=28){
        if(rate>=22)
         rate = 12;
        else
         rate = 0;
       }else if(rate>=39&&rate<46){
         rate =33;
       }
    }
...
{	   
	...
	if (pwm_set_rate(dcdc->pdata,1000*1000,pwm_value,dcdc->desc.id)!=0)
	{
...
static int __devinit pwm_regulator_probe(struct platform_device *pdev)
{
    ...
    if(pdev->id==0||pdev->id==1) 
      ...
      printk("pwm_regulator id is not 0 or 1\n");
    }
