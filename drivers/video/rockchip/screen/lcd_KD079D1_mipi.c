...
#define OUT_FACE	    	OUT_P666 //OUT_P888//OUT_P666 //OUT_P888
...
int rk_lcd_init(void) {
	...
	if(gTmp == 1)
	{
		...
		ret = gpio_request(RK30_PIN1_PB2, "mipi power pin");
		if( ret != 0 )
		{
			gpio_free(RK30_PIN1_PB2);
			printk("mipi power pin error\n");
			return -EIO;
		}
		gTmp++;

        gpio_set_value(RK30_PIN1_PB2, GPIO_LOW);
	    msleep(100);
    }
        gpio_set_value(RK30_PIN1_PB2, !GPIO_LOW);
        msleep(3);
