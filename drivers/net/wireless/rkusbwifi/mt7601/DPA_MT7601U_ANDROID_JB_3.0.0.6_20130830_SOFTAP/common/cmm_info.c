...
#if defined(DPA_S) && defined(P2P_SUPPORT)
	...
	IN	PSTRING			arg)
{
	...
	pAd->CommonCfg.FccaThr1 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(THR1=%d)\n", __FUNCTION__, pAd->CommonCfg.FccaThr1));
	return TRUE;
}

INT Set_FalseCCA_THR2_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	UINT16 Value;
	...
	Value = (UINT16)simple_strtol(arg, 0, 10);
	pAd->CommonCfg.FccaThr2 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(THR2=%d)\n", __FUNCTION__, pAd->CommonCfg.FccaThr2));
	return TRUE;
}

INT Set_RSSI_THR1_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	INT16 Value;
	...
	Value = (UINT16)simple_strtol(arg, 0, 10);
	pAd->CommonCfg.RssiThr1 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(Rssi Thr1=%d)\n", __FUNCTION__, pAd->CommonCfg.RssiThr1));
	return TRUE;
}

INT Set_RSSI_THR2_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	INT16 Value;
	...
	Value = (UINT16)simple_strtol(arg, 0, 10);
	pAd->CommonCfg.RssiThr2 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(Rssi Thr2=%d)\n", __FUNCTION__, pAd->CommonCfg.RssiThr2));
	return TRUE;
}

INT Set_RSSI_THR3_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	INT16 Value;
	...
	Value = (UINT16)simple_strtol(arg, 0, 10);
	pAd->CommonCfg.RssiThr3 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(Rssi Thr3=%d)\n", __FUNCTION__, pAd->CommonCfg.RssiThr3));
	return TRUE;
}

INT Set_RSSI_NOISE_THR1_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	INT16 Value;
	...
	Value = (UINT16)simple_strtol(arg, 0, 10);
	pAd->CommonCfg.NoiseRssiThr1 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(Rssi Thr1=%d)\n", __FUNCTION__, pAd->CommonCfg.NoiseRssiThr1));
	return TRUE;
}

INT Set_RSSI_NOISE_THR2_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	INT16 Value;
	...
	Value = (UINT16)simple_strtol(arg, 0, 10);
	pAd->CommonCfg.NoiseRssiThr2 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(Rssi Thr2=%d)\n", __FUNCTION__, pAd->CommonCfg.NoiseRssiThr2));
	return TRUE;
}

INT Set_RSSI_NOISE_THR3_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	INT16 Value;
	...
	Value = (UINT16)simple_strtol(arg, 0, 10);
	pAd->CommonCfg.NoiseRssiThr3 = Value;
	DBGPRINT(RT_DEBUG_TRACE, ("IF %s::(Rssi Thr3=%d)\n", __FUNCTION__, pAd->CommonCfg.NoiseRssiThr3));
	return TRUE;
}

#endif /*#ifdef DYNAMIC_PD_SUPPORT*/

/*
==============================================
 Description: 
     If RT2870STA.dat does not specify value, 
     need to set default thredthod for dynamic VGA
 Parameters:
 Note: different chip may use different value
==============================================
*/
#ifdef DYNAMIC_PD_SUPPORT
#define SET_DEFAULT_THRESHOLD(name, value) \
	do { \
		if (!name) \
			name = value; \
	} while (0)

VOID Set_PD_threshold_Proc(
	IN	PRTMP_ADAPTER	pAd)
{
	if (IS_MT7601(pAd))
	{
		/*RSSI thredshold for fundamental RSSI-based Dynamic VGA*/
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.RssiThr1, -45);
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.RssiThr2, -60);
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.RssiThr3, -75);

    /*RSSI thredshold for fundamental anti-noise Dynamic VGA*/
    /*Enhance short & middle distance vga effect*/
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.NoiseRssiThr1, -62);
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.NoiseRssiThr2, -70);
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.NoiseRssiThr3, -75);

    /*FCCA thredshold*/
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.FccaThr1, 800);
		SET_DEFAULT_THRESHOLD(pAd->CommonCfg.FccaThr1, 10);

		/*Register initial*/
		pAd->CommonCfg.InitR66 = 0x14;
    pAd->CommonCfg.InitR68 = 0x9C;
    pAd->CommonCfg.InitR195= 0x87;
    pAd->CommonCfg.InitR196= 0x18; /*max:0x28*/
...
==============================================
 Description: 
 	Enable dynamic VGA & Periodic monitor
 Parameters:
     1 means enable; 0 means disable
==============================================
*/
#ifdef DYNAMIC_PD_SUPPORT
INT Set_Dynamic_PD_Enable_Proc(
	IN	PRTMP_ADAPTER	pAd, 
	IN	PSTRING			arg)
{
	UINT16 Value;
	...
	if (Value != 0)
	{
		pAd->CommonCfg.bDynaPDEnable = TRUE;
#ifdef ANTI_NOISE_PD_SUPPORT
    pAd->CommonCfg.bAntDivEnable = FALSE;
#endif /*ANTI_NOISE_PD_SUPPORT*/

		/*set default threshold for dynamic VGA value*/
		Set_PD_threshold_Proc(pAd);
	}
	else
	{
		pAd->CommonCfg.bDynaPDEnable = FALSE;
#ifdef ANTI_NOISE_PD_SUPPORT
  /*reopen ant div*/
    if (pAd->CommonCfg.bAntDivEnable == 1)
    {
      ...
      DBGPRINT(RT_DEBUG_TRACE, ("[%s]Set_Antenna_Proc(pAd, 1)\n", __FUNCTION__));
      
    }
#endif /*ANTI_NOISE_PD_SUPPORT*/

		/*Restore default R66 & R68*/
	...
	DBGPRINT(RT_DEBUG_TRACE, ("IF Set_Dynamic_PD_Enable_Proc::(Enable =%d)\n", 
  pAd->CommonCfg.bDynaPDEnable));
	return TRUE;
}
#endif /*DYNAMIC_PD_SUPPORT*/
