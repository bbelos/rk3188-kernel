	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen + 7 - sizeof(P2P_PUBLIC_FRAME)), 
		FALSE, &Dpid, &GroupCap, &DeviceCap, NULL, &DeviceNameLen, DevAddr, IfAddr, NULL, &SsidLen, (PUCHAR)&Ssid, &ConfigMethod, NULL, DevType, &Channel, &OpChannel, NULL, &Intent, &StatusCode, NULL, NULL);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8),  
		...
		DevAddr, IfAddr, NULL, &SsidLen, Ssid, &ConfigMethod, NULL, DevType, &Channel, &OpChannel, NULL, &TempIntent, &StatusCode, NULL, NULL);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
		...
		*/
		...
		P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8),
			...
			BssidAddr, NULL, &SsidLen, NULL, &ConfigMethod, NULL, DevType, &Channel, &OpChannel, NULL, &TempIntent, &StatusCode, NULL, NULL);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), 
		...
		BssidAddr, &SsidLen, NULL, NULL, NULL, NULL, NULL, &OpChannel, NULL, NULL, &StatusCode, NULL, NULL);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), 
		...
		BssidAddr, &SsidLen, Ssid, &ConfigMethod, NULL, DevType, NULL, &OpChannel, ChannelList, NULL, &StatusCode, &InviteFlag, NULL);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), 
		FALSE, NULL, &GroupCap, &DeviceCap, NULL, &DeviceNameLen, devAddr, NULL, 
		BssidAddr, &SsidLen, NULL, &ConfigMethod, NULL, DevType, NULL, &OpChannel, ChannelList, NULL, &StatusCode, NULL, NULL);
	...
	IN MLME_QUEUE_ELEM *Elem) 
{
	...
	P2pParseSubElmt(pAd, &pFrame->ElementID, (Elem->MsgLen - LENGTH_802_11 - 8), 
		...
		BssidAddr, &SsidLen, Ssid, NULL, &WpsConfigMethod, DevType, NULL, &OpChannel, NULL, NULL, NULL, NULL, NULL);
									...
									P2pSubelement))
		{
			...
			if (bBeacon == FALSE)
			{
				P2pParseSubElmt(pAd, (PVOID)P2pSubelement, P2PSubelementLen, 
					bBeacon, &Dpid, &GroupCap, &DeviceCap, DeviceName, &DeviceNameLen, DevAddr, NULL, NULL, NULL, NULL, &ConfigMethod, NULL, DevType, &Channel, &OpChannel, NULL, &Intent, &StatusCode, NULL, NULL);
			}
			else
				P2pParseSubElmt(pAd, (PVOID)P2pSubelement, P2PSubelementLen, 
					bBeacon, &Dpid, &GroupCap, &DeviceCap, DeviceName, &DeviceNameLen, DevAddr, NULL, NULL, NULL, NULL, NULL, NULL, DevType, &Channel, &OpChannel, NULL, &Intent, &StatusCode, NULL, NULL);
...
//QQ TBD, p2p widi need insert p2p entry in probe req. due to p2p0 only in listen state
			...
			if (index < MAX_P2P_GROUP_SIZE)
			{
				P2pParseSubElmt(pAd, P2pSubelement, P2PSubelementLen, 
					FALSE, NULL, &GroupCap, &DeviceCap, NULL, NULL, DevAddr, NULL, NULL, &SsidLen, Ssid, NULL, NULL, pAd->P2pTable.Client[index].PrimaryDevType, &pAd->P2pTable.Client[index].ListenChannel, &pAd->P2pTable.Client[index].OpChannel, NULL, &pAd->P2pTable.Client[index].GoIntent, &StatusCode, NULL, NULL);
