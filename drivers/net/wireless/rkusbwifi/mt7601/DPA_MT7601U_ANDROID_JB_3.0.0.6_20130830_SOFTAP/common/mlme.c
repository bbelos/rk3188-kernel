	...
	IN UCHAR			Property)
{
	...
	/* look for all registered channels */
	for(IdChan = 0; IdChan < pP2PCtrl->ChannelListNum; IdChan++)
	{
		if (pP2PCtrl->ChannelList[IdChan].Channel == ChanNum)
		{
			if ((pP2PCtrl->ChannelList[IdChan].Flags & Property) == Property)
				return TRUE;
...
#else
#ifdef CONFIG_AP_SUPPORT
		/*	((pAd->SameRxByteCount > 5) && (pAd->BbpResetCount < 2)) */
			(pAd->SameRxByteCount > 0)
#endif /* CONFIG_AP_SUPPORT */
#endif /* P2P_SUPPORT */
		)
