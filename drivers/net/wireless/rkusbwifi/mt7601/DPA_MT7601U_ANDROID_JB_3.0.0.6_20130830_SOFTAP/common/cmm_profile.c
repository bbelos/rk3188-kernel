...
/*	tmpbuf = kmalloc(MAX_PARAM_BUFFER_SIZE, MEM_ALLOC_FLAG);*/
    ...
    if (RTMPGetKeyParameter("FCCA_THR1", tmpbuf, 10, pBuffer, TRUE))
      Set_FalseCCA_THR1_Proc(pAd, tmpbuf);
    if (RTMPGetKeyParameter("FCCA_THR2", tmpbuf, 10, pBuffer, TRUE))
      Set_FalseCCA_THR2_Proc(pAd, tmpbuf);

    /*Set false RSSI thredshold(for findamental RSSI-Base Dynamic VGA)*/
    if (RTMPGetKeyParameter("RSSI_THR1", tmpbuf, 10, pBuffer, TRUE))
      Set_RSSI_THR1_Proc(pAd, tmpbuf);
    if (RTMPGetKeyParameter("RSSI_THR2", tmpbuf, 10, pBuffer, TRUE))
      Set_RSSI_THR2_Proc(pAd, tmpbuf);
    if (RTMPGetKeyParameter("RSSI_THR3", tmpbuf, 10, pBuffer, TRUE))
      Set_RSSI_THR3_Proc(pAd, tmpbuf);

    /*Set false RSSI thredshold(for Anti-Noise RSSI-Base Dynamic VGA)*/
    if (RTMPGetKeyParameter("NOISE_RSSI_THR1", tmpbuf, 10, pBuffer, TRUE))
      Set_RSSI_NOISE_THR1_Proc(pAd, tmpbuf);
    if (RTMPGetKeyParameter("NOISE_RSSI_THR2", tmpbuf, 10, pBuffer, TRUE))
      Set_RSSI_NOISE_THR1_Proc(pAd, tmpbuf);
    if (RTMPGetKeyParameter("NOISE_RSSI_THR3", tmpbuf, 10, pBuffer, TRUE))
      Set_RSSI_NOISE_THR1_Proc(pAd, tmpbuf);

    /*1:enable dynamic vga. 0: disable dynamic vga*/
    if (RTMPGetKeyParameter("DYPD_ENABLE", tmpbuf, 10, pBuffer, TRUE))
      Set_Dynamic_PD_Enable_Proc(pAd, tmpbuf);
	...
	IN UCHAR			bw)
{
	...
	if ( rate_pwr > pAd->DefaultTargetPwr )
		rate_pwr = pAd->DefaultTargetPwr;
	...
	IN CHAR				base_pwr)
{
	...
	for ( i = 0 ; i < 5; i++ )
	{
		...
		data = (rate_table[i*4] & 0x3F )+ ((rate_table[i*4 + 1] << 8) & 0x3F);
		...
		if ( i != 4 )
		{
			data |= ((rate_table[i*4 + 2] << 16) & 0x3F )+ ((rate_table[i*4 + 3] & 0x3F) << 24);
