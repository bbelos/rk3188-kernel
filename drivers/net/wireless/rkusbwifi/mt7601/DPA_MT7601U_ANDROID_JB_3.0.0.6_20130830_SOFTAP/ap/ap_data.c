...
VOID APSendPackets(NDIS_HANDLE dev_hnd, PPNDIS_PACKET pkt_list, UINT pkt_cnt)
{
	...
	for (Index = 0; Index < pkt_cnt; Index++)
	{
		...
		*/
		...
		if (RTMP_GET_PACKET_NET_DEVICE(pPacket) >= MIN_NET_DEVICE_FOR_CFG80211_VIF_P2P_GO)
			allowToSend = ApAllowToSendPacket(pAd, pPacket, &wcid);
		else if (RTMP_GET_PACKET_NET_DEVICE(pPacket) >= MIN_NET_DEVICE_FOR_CFG80211_VIF_AP)
			allowToSend = ApAllowToSendPacket(pAd, pPacket, &wcid);
		else
	...
	if (IS_ENTRY_CLIENT(pMacEntry) || (Wcid == MCAST_WCID))
	{
		...
		if (Wcid == MCAST_WCID)
		{
			...
			if ((apidx == MIN_NET_DEVICE_FOR_CFG80211_VIF_P2P_GO) ||
			   (apidx == MIN_NET_DEVICE_FOR_CFG80211_VIF_AP))	
				apidx = MAIN_MBSSID;
#else
#ifdef P2P_SUPPORT
	...
	IN RX_BLK *pRxBlk)
{
	...
	if ((pHeader->FC.SubType == SUBTYPE_PROBE_REQ) || 
	    ...
	    (CFG80211DRV_PrintFrameType(pAd, "P2P_CHECK", pHeader,  pRxWI->RXWI_O.MPDUtotalByteCnt) == TRUE)))
	{
		UINT32 freq;
		...
		if (pAd->Cfg80211VifDevSet.vifDevList.size > 0)        
		{
			...
			{	 
				if (CFG80211OS_RxMgmt(pNetDev, freq, (PUCHAR)pHeader, pRxWI->RXWI_O.MPDUtotalByteCnt))
				{
					DBGPRINT(RT_DEBUG_INFO, ("VIF AP GO RtmpOsCFG80211RxMgmt OK!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
								  pHeader->FC.SubType, freq, PRINT_MAC(pHeader->Addr2)));
					return;
				}	
				else
				DBGPRINT(RT_DEBUG_INFO, ("VIF AP GO RtmpOsCFG80211RxMgmt FAIL!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
                                                                  pHeader->FC.SubType, freq, PRINT_MAC(pHeader->Addr2)));			
			}
		...
		{	 
			...
			{
				DBGPRINT(RT_DEBUG_INFO, ("MAIN AP RtmpOsCFG80211RxMgmt OK!! TYPE = %d, freq = %d, %02x:%02x:%02x:%02x:%02x:%02x\n",
							  pHeader->FC.SubType, freq, PRINT_MAC(pHeader->Addr2)));	
				return;			
			}
			else
	...
	*/
	...
	else if ((pFmeCtrl->FrDs == 1) && (pFmeCtrl->ToDs == 0))
	{
		...
		if (pEntry && IS_ENTRY_APCLI(pEntry))
		{
			...
			{
				/* for AP detection purpose, not only Beacon but also unicast data */
				NdisGetSystemUpTime(&pAd->ApCfg.ApCliTab[pEntry->MatchAPCLITabIdx].ApCliRcvBeaconTime);
...
#ifdef P2P_SUPPORT
			//Fix two station can't ping successfully when p2p go as softap.
	...
	IN USHORT PwrMgmt)
{
	...
	//COPY_MAC_ADDR(pHeader_802_11->Addr1, pAd->ApCliMlmeAux.Bssid);
	COPY_MAC_ADDR(pHeader_802_11->Addr2, pApCliEntry->CurrentAddress);
	//COPY_MAC_ADDR(pHeader_802_11->Addr3, pAd->ApCliMlmeAux.Bssid);
