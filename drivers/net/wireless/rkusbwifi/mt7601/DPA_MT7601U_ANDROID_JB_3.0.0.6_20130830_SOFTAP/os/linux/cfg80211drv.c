	...
	BOOLEAN                                          isAdd)
{
	...
	wpa_ie = cfg80211_find_ie(WLAN_EID_VENDOR_SPECIFIC, pBeacon->beacon_tail, 30); //if it doesn't find WPA_IE in tail first 30 bytes. treat it as is not found.
#else
		...
		{			
			...
			{		
				...
				{		
					...
					RTMPZeroMemory(pVendorIE + 2, VendorIELen);				
					pPreVendorIE = pVendorIE;
					pVendorIE += *(pVendorIE + 1) + 2; //Next Vendor IE address pointer
					  
					if (pVendorIE < pBeacon->beacon_tail + pBeacon->beacon_tail_len)
					{					
						RTMPMoveMemory(pPreVendorIE, pVendorIE , pBeacon->beacon_tail + pBeacon->beacon_tail_len - pVendorIE);						
					}

					//Reset beacon length	
					pBeacon->beacon_tail_len -= (VendorIELen + 2);
