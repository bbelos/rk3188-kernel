  ...
  IN RTMP_ADAPTER   *pAd)
{
  ...
  UINT32         rssi             = 0;
  UINT32         pre_reg_val      = 0;
  UINT32         pre_reg_val_aux  = 0;

  INT16 RSSI_THR1 = pAd->CommonCfg.RssiThr1; //Default: -45dBm
  INT16 RSSI_THR2 = pAd->CommonCfg.RssiThr2; //Default: -60dBm
  INT16 RSSI_THR3 = pAd->CommonCfg.RssiThr3; //Default: -75dBm
  ...
  if (pAd->ApCfg.MBSSID[MAIN_MBSSID].StaCount < 2)
  {
    /*get default rssi_thr*/  
      if (rssi > RSSI_THR1)//~-45
      {
        init_main_vga += 0x30;
        init_aux_vga += 0x30;
      }
      else if ((rssi <= RSSI_THR1) && (rssi > RSSI_THR2))//-45~-60
      {
        init_main_vga += 0x20;
        init_aux_vga += 0x20;
      }
      else if ((rssi <= RSSI_THR2) && (rssi > RSSI_THR3))//-60~-75
      {
        init_main_vga += 0x10;
        init_aux_vga += 0x10;
      }
...
VOID MT7601_Periodic_Monitor_FalseCCA(
  ...
  UINT32                *aux_vga)
{
	UINT16 FCCA_THR1 		  = pAd->CommonCfg.FccaThr1; //Default: 800
	UINT16 FCCA_THR2 		  = pAd->CommonCfg.FccaThr2; //Default: 10
  UINT32 FCCACnt        = pAd->RalinkCounters.OneSecFalseCCACnt;
  UINT32 Init_main_vga  = pAd->CommonCfg.InitR66;
  UINT32 Init_aux_vga   = pAd->CommonCfg.InitR196;

  /*Check False CCA*/             
  if (FCCACnt > FCCA_THR1) //Defaut: 800
  {
    *main_vga += 0x04; //-2dBm
  ...
  else if (FCCACnt < FCCA_THR2) //Default: 10
  {
    *main_vga -= 0x04; //+2dBm
